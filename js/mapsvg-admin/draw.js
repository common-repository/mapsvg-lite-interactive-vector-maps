    ...
    MapSVGAdminDrawController.prototype.setEventHandlers = function(){
        ...
        if(_this.readOnly){
            $('body').on('click.mapsvg-save-copy', '#mapsvg-save-svg-copy',function(){
                ...
                $.post(ajaxurl, {action: 'mapsvg_svg_copy', filepath: _this.filepath, _wpnonce: mapsvg_nonce.nonce})
                    .done(function(data){
    ...
    MapSVGAdminDrawController.prototype.saveSvg = function(){
        ...
        $.get(_this.filepath + '?v=' + Math.random())
            .done(function(xmlData){
                ...
                $.post(ajaxurl, {action: 'mapsvg_save_svg', _wpnonce: mapsvg_nonce.nonce, map_id: _this.mapsvg.id, body: newSVG[0].outerHTML.replace(/\sgeoViewBox/ig,' mapsvg:geoViewBox'), filepath: _this.filepath})
                    .done(function(data){
                        newSVG = null;
